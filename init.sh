...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/l-ff/Docker-for-Nezha-Argo-server-v0.x/main/README.md >/dev/null 2>&1 && unset GH_PROXY
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/l-ff/Docker-for-Nezha-Argo-server-v0.x/raw/main/sqlite.db
 fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/l-ff/Docker-for-Nezha-Argo-server-v0.x/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/l-ff/Docker-for-Nezha-Argo-server-v0.x/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/l-ff/Docker-for-Nezha-Argo-server-v0.x/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh
...
# 启动xxxry
wget -qO- https://github.com/l-ff/d/releases/download/sd/kano-6-amd-w > $WORK_DIR/webapp
